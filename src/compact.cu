...
__global__ void compact_kernel(int* d_in, int* d_out, int* d_predicate, int size)
{
...
__global__ void scatter_kernel(int* d_in, int* d_out, int* d_predicate, int size)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
void compact(int* d_in, int* d_out, int* d_predicate, int size) {
    ...
    thrust::exclusive_scan(thrust::device, d_predicate, d_predicate + size, d_predicate);
    
    scatterKernel<<<blocksPerGrid, threadsPerBlock>>>(d_in, d_out, d_predicate, size);
    cudaDeviceSynchronize();
}
